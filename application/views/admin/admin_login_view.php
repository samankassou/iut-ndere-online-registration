  ...
  <div class="card">
    <div class="card-body login-card-body">
      ...
      <form action="<?= base_url() ?>login/login_validation" method="post">
        ...
        <div class="input-group mb-3 <?= empty(form_error('password'))?'':'has-error' ?>">
          <input type="password" class="form-control" name="password" value="<?= set_value('password') ?>" placeholder="Mot de passe">
          <div class="input-group-append">
            <div class="input-group-text">
        ...
        <div class="help-block text-light text-center <?= empty(validation_errors())?'':'alert-danger'?> <?= (!empty($error))?'alert-danger':'' ?>" id="error">
          <small><?= validation_errors() ?></small><p><?= (!empty($error))?$error:'' ?></p>
        </div>
        <div class="row">
          <div class="col-7">
